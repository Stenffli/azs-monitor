...
const PRICES_BACKUP_PATH = path.join(__dirname, 'prices_backup.json');
let db;

// ============================================================
// СОХРАНЕНИЕ ЦЕН В JSON-ФАЙЛ
// ============================================================
function savePricesToBackup() {
    if (!db) return;
    db.all("SELECT station_id, fuel_type, price, availability FROM fuel_stock", (err, rows) => {
        if (err) {
            console.error('❌ Ошибка чтения цен для бэкапа:', err);
            return;
        }
        fs.writeFileSync(PRICES_BACKUP_PATH, JSON.stringify(rows, null, 2));
        console.log(`💾 Сохранено ${rows.length} цен в бэкап`);
    });
}

// ============================================================
// ВОССТАНОВЛЕНИЕ ЦЕН ИЗ БЭКАПА
// ============================================================
function restorePricesFromBackup() {
    if (!fs.existsSync(PRICES_BACKUP_PATH)) {
        console.log('ℹ️ Бэкап цен не найден');
        return false;
    }
    try {
        const data = JSON.parse(fs.readFileSync(PRICES_BACKUP_PATH));
        if (data.length === 0) return false;
        
        // Восстанавливаем цены в базу
        const stmt = db.prepare(`
            INSERT OR REPLACE INTO fuel_stock (station_id, fuel_type, price, availability)
            VALUES (?, ?, ?, ?)
        `);
        data.forEach(row => {
            stmt.run(row.station_id, row.fuel_type, row.price, row.availability);
        });
        stmt.finalize();
        console.log(`✅ Восстановлено ${data.length} цен из бэкапа`);
        return true;
    } catch (e) {
        console.error('❌ Ошибка восстановления цен:', e);
        return false;
    }
}
    ...
    db.serialize(() => {
        ...
        db.get("SELECT COUNT(*) as count FROM gas_stations", (err, row) => {
            if (row.count === 0) {
                ...
                // ===== ВОССТАНАВЛИВАЕМ ЦЕНЫ ИЗ БЭКАПА =====
                const restored = restorePricesFromBackup();
                if (!restored) {
                    // Если бэкапа нет — ставим дефолтные цены
                    ...
                    console.log(`✅ Цены восстановлены из бэкапа`);
                }
            } else {
...
app.post('/api/report', (req, res) => {
    ...
    `, [station_id, user_name || 'Аноним', report_type, fuel_type || null,
        ...
        // ===== СОХРАНЯЕМ ЦЕНЫ В БЭКАП ПОСЛЕ КАЖДОГО ИЗМЕНЕНИЯ =====
        savePricesToBackup();
